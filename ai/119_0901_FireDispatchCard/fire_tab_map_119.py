...
def apply_bert_subtype_to_case(case: Any, label: str, conf: float) -> None:
    """
    將 BERT 細類寫入 case。fire_tab 已鎖定時只更新該垂片內的細類碼，不改路由。
    """
    ...
    if locked_tab:
        if predicted_tab and predicted_tab != locked_tab:
            return
        if locked_tab == TAB_A and codes.get("building_type_code"):
            case.building_type_code = codes["building_type_code"]
        elif locked_tab in (TAB_B1, TAB_B2) and codes.get("vehicle_wildfire_code") is not None:
            case.vehicle_wildfire_code = codes["vehicle_wildfire_code"]
        elif locked_tab == TAB_C and codes.get("minor_fire_code") is not None:
            case.minor_fire_code = codes["minor_fire_code"]
        return
    ...
    if existing_type is None and predicted_type is not None:
        case.fire_incident_type = predicted_type
    if getattr(case, "non_building_fire", None) is None and codes.get("non_building_fire") is not None:
        case.non_building_fire = codes["non_building_fire"]
    if codes.get("building_type_code"):
        case.building_type_code = codes["building_type_code"]
    if codes.get("vehicle_wildfire_code") is not None:
        case.vehicle_wildfire_code = codes["vehicle_wildfire_code"]
    if codes.get("minor_fire_code") is not None:
        case.minor_fire_code = codes["minor_fire_code"]
