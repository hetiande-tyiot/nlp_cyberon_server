...
class FireTabDialogueTests(unittest.TestCase):
    ...
    def test_q1_unresolved_tags_and_continues(self) -> None:
        engine, io, _ = _run([
            "不清楚啊",
            "還是不清楚",
            "都不是，是垃圾在燒",
        ...
        asked = "\n".join(io.messages)
        self.assertNotIn("轉接專人", asked)
        self.assertIn("Q1 兩輪仍無法判斷", engine.case.ImportantTag)
    ...
    def test_q2_unresolved_defaults_to_tab_c(self) -> None:
        ...
        self.assertIn("Q2 兩輪仍無法判斷", engine.case.ImportantTag)
        self.assertEqual(engine.case.ImportantCase, 1)
        self.assertNotIn("轉接專人", "\n".join(io.messages))
        self.assertIn(SAFETY_MESSAGE, "\n".join(io.messages))

    def test_caller_info_missing_tags_and_dispatches(self) -> None:
        engine, io, _ = _run([
            "不是房子",
            "都不是，是垃圾在燒",
            "不知道",
        ...
        asked = "\n".join(io.messages)
        self.assertNotIn("轉接專人", asked)
        self.assertIn("回撥電話無法確認", engine.case.ImportantTag)
        self.assertIn("報案人稱呼無法確認", engine.case.ImportantTag)
        self.assertEqual(engine.case.ImportantCase, 1)
        self.assertIsNone(engine.case.caller_contact)
        self.assertIsNone(engine.case.caller_salutation)
        self.assertEqual(engine.case.result, "dispatched")
        self.assertEqual(engine.case.fire_tab, TAB_C)
