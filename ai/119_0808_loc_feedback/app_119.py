...
_STAGE_LABELS: List[tuple[str, str]] = [
    ...
    ("火警_initial_1",           "火/煙/氣味研判"),
    ("火警_initial_smoke_color", "確認煙色"),
    ("火警_initial_2",           "確認燃燒物"),
    ("火警_initial_3",           "確認火勢與範圍"),
    ("火警_category",            "判斷燃燒標的"),
    ("火警_building_identity",   "建物：位置與身分"),
    ("火警_building_trapped",    "建物：人員受困"),
    ("火警_building_floors",     "建物：樓層"),
    ("火警_building_spread",     "建物：延燒風險"),
    ("火警_factory_scale",       "工廠：規模類型"),
    ("火警_factory_trapped",     "工廠：人員受困"),
    ("火警_factory_hazard",      "工廠：危險物品"),
    ("火警_vehicle_detail",      "車輛：車種數量"),
    ("火警_vehicle_occupants",   "車輛：車內人員"),
    ("火警_vehicle_road",        "車輛：道路位置"),
    ("火警_outdoor_type",        "野外：燃燒場域"),
    ("火警_outdoor_water",       "野外：附近水源"),
    ("火警_outdoor_threat",      "野外：波及目標"),
...
_FIRE_PREFIX: List[str] = [
    ...
    "location_validation_reask",
    "火警_location_confirm",
    "火警_initial_1", "火警_initial_smoke_color",
    "火警_initial_2", "火警_initial_3", "火警_category",
]
_FIRE_BRANCH_STAGES: Dict[str, List[str]] = {
    "建築物": [
        "火警_building_identity", "火警_building_trapped",
        "火警_building_floors", "火警_building_spread",
    ],
    "工廠": ["火警_factory_scale", "火警_factory_trapped", "火警_factory_hazard"],
    "車輛": ["火警_vehicle_detail", "火警_vehicle_occupants", "火警_vehicle_road"],
    "露天野外": ["火警_outdoor_type", "火警_outdoor_water", "火警_outdoor_threat"],
}
...
def _get_visible_stages(
    ...
    if main_cat is None:
        ...
        keys = _FIRE_PREFIX + _FIRE_BRANCH_STAGES.get(sub_cat or "", []) + _FIRE_SUFFIX
    elif main_cat == "緊急救援":
    ...
    with right_col:
        ...
        with st.container():
            ...
            if case.get("address_suspect_error"):
                ...
                st.error(f"地址抽取或校驗錯誤，已轉接專人：{reason}")
        ...
        with st.container():
            ...
            fire_fields = [
                ...
                ("火災類別",   case.get("fire_category")),
            ]
            fire_category = case.get("fire_category")
            if fire_category == "建築物":
                fire_fields += [
                    ("報案人位置", case.get("caller_position")),
                    ("報案人身分", case.get("caller_role")),
                    ("是否受困", case.get("people_trapped")),
                    ("受困人數", case.get("trapped_count")),
                    ("建物總樓層", case.get("building_total_floors")),
                    ("起火樓層", case.get("fire_floor")),
                    ("是否延燒", case.get("fire_spread")),
                    ("建物型態", case.get("building_layout")),
                ]
            elif fire_category == "工廠":
                fire_fields += [
                    ("工廠規模", case.get("factory_scale_type")),
                    ("廠內有人", case.get("factory_people_present")),
                    ("是否受困", case.get("people_trapped")),
                    ("受困人數", case.get("trapped_count")),
                    ("危險物品", case.get("hazardous_materials")),
                ]
            elif fire_category == "車輛":
                fire_fields += [
                    ("車種", case.get("vehicle_type")),
                    ("車輛數", case.get("vehicle_count")),
                    ("車內有人", case.get("vehicle_occupants")),
                    ("車內人數", case.get("vehicle_occupant_count")),
                    ("道路位置", case.get("road_type")),
                ]
            elif fire_category == "露天野外":
                fire_fields += [
                    ("燃燒場域", case.get("outdoor_fire_type")),
                    ("附近水源", case.get("nearby_water_source")),
                    ("波及目標", case.get("affected_targets")),
                    ("是否受困", case.get("people_trapped")),
                ]
            rows_html = ""
        ...
        with st.container():
            ...
            steps_html = ""
            visible_stages = _get_visible_stages(main_cat, sub_cat)
