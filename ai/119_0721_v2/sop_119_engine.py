...
class SopEngine119:
    ...
    def _run_救護(self) -> None:
        ...
        # ════════════════════════════════════════════════════════
        # 階段 1：詢問地址（null/未知至多問 2 次）→ 有可用地址再複讀確認
        # ════════════════════════════════════════════════════════
        self._set_stage("救護_location")

        # 先從歷史輸入（含第一輪）補抽地址；已有可用地址則跳過詢問
        ...
        addr_asks = 0
        while not is_usable_address(self.case.address) and addr_asks < _ADDR_ASK_MAX:
            addr_q = _addr_ask_questions[min(addr_asks, len(_addr_ask_questions) - 1)]
            addr_asks += 1
            self._debug_print("address_ask", {"n": addr_asks, "q": addr_q})
            answer = self._ask_and_extract(addr_q)
            self._clear_unusable_address()
            # 通用抽取未拿到可用地址時，再跑專項地址抽取
            if not is_usable_address(self.case.address):
        ...
        if is_usable_address(self.case.address):
            ...
            # 問滿 2 次仍無可用地址 → 轉人工（禁止複讀「未知」或繼續 SOP）
