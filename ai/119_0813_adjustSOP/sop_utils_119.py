...
═══════════════════════════════════════════════════════════════════
我們自己加的東西（從 0808 版搬過來，廠商的 0813 沒有）
═══════════════════════════════════════════════════════════════════

【一、報案人沒說縣市時，自動補上「新北市」】

  為什麼要加：
    查地址的那個外部系統，一定要有縣市才查得到。
    報案人說「板橋區中山路一段161號」——地址完全正確，只是沒說縣市——
    那個系統就回查不到，我們的程式就會跟報案人說「地址無法確認，請重講」。
    但板橋人打電話本來就不會特地說「新北市」，這樣很不合理。

  怎麼改：
    在拿地址去查之前，先看看有沒有縣市。沒有就自動加上「新北市」。
    要改預設縣市的話，設環境變數 DEFAULT_CITY_119 就好，不用改程式。

  為什麼要有縣市清單（TAIWAN_CITIES）：
    不能只看有沒有「市」這個字。「士林夜市」「中央市場」都有「市」，
    但那不是縣市名。所以列出全台 22 個縣市，一個一個比對才準。

  相關的東西：TAIWAN_CITIES、DEFAULT_CITY、has_city_prefix()、ensure_city_prefix()

【二、在戶外的案件，不要問「幾樓」】

  為什麼要加：
    廠商也有做這件事，但他們只認得車禍、路倒這兩類的說法。
    報案人說「有人昏倒在天橋下」，廠商認不出來，還是會問「幾樓？」——
    天橋沒有樓層，這樣問很奇怪。

  怎麼改：
    把 46 個戶外地點的說法（巷口、天橋、路邊、公園、高速公路…）
    加進廠商原本的清單裡。只加詞，沒有改廠商的程式邏輯。

  相關的東西：OUTDOOR_ADDRESS_NO_FLOOR_KEYWORDS 底下的 _LOCAL_OUTDOOR_KEYWORDS

【沒有搬過來的東西】

  0808 還有一組「地址分好幾句講、要合併起來」的功能，這次沒搬。
  因為廠商在 0813 自己做了一套（merge_address 那些），而且做得比我們細，
  硬要兩套併在一起反而會打架。
...
# ═══ 我們加的：報案人沒說縣市時，自動補上 ═══════════════════════════
# 預設補哪個縣市。想改的話設環境變數 DEFAULT_CITY_119，不用動程式。
DEFAULT_CITY = os.environ.get("DEFAULT_CITY_119", "新北市")

# 全台縣市清單。用來判斷「這個地址有沒有講縣市」。
# 不能只看「市」這個字，因為「士林夜市」「中央市場」也有「市」但不是縣市名。
TAIWAN_CITIES: Tuple[str, ...] = (
...
def ensure_city_prefix(addr: str, city: Optional[str] = None) -> str:
    ...
    例：「板橋區中山路一段161號」→「新北市板橋區中山路一段161號」
    已經有縣市的話原樣不動。
    """
    s = str(addr or "").strip()
...
# ═══ 我們加的：更多戶外地點的說法 ═══════════════════════════════════
# 廠商原本只認得車禍、路倒這兩類的說法。
# 但報案人說「有人昏倒在天橋下」，廠商認不出來，還是會問「幾樓？」。
# 所以補上這 46 個明顯在戶外、不可能有樓層的說法。
_LOCAL_OUTDOOR_KEYWORDS: Tuple[str, ...] = (
...
def extract_address_number(text: str) -> Optional[str]:
    """抽取門牌號並統一使用繁體「號」。"""
    compact = re.sub(r"\s+", "", text or "")
    match = _ADDRESS_NUMBER_RE.search(compact)
