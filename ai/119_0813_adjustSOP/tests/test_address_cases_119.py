...
中途更正。本檔以資料表列出各種說法與期望結果：
  ...
  OUTDOOR_CASES  戶外案件不追問樓層（address_ask_should_include_floor）

多輪對話流程（元件式補問、覆誦、MapaddrCheck /Verify 驗測）由
tests/test_address_flow_119.py 覆蓋，本檔不重複。
"""
...
class OutdoorCaseTests(unittest.TestCase):
    def test_outdoor_hint_cases(self) -> None:
        for cid, text, outdoor in OUTDOOR_CASES:
            with self.subTest(case=f"{cid} {text}"):
                self.assertIs(address_ask_should_include_floor(text), not outdoor)

    def test_ask_question_building(self) -> None:
        for cid, include_floor, expected_first in ASK_QUESTION_CASES:
            with self.subTest(case=cid):
                questions = build_address_ask_questions(
                    include_floor=include_floor, flow="救護"
                )
                self.assertEqual(questions[0], expected_first)
